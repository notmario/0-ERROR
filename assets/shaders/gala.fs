...
extern MY_HIGHP_OR_MEDIUMP vec2 gala;
...
float star(float xcor, float ycor, float curve, float size)
{
    float sBase = size - pow(pow(abs(xcor), curve) + pow(abs(ycor), curve), 1/curve);
    return(min(1, max(0, sBase)));
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float cy = uv_scaled_centered.y * 95/71;
    ...
    {
        sw1 += 20/swirlclumps * msin(i + t/20) * 
            swirl(
                cx/3 + 7 * msin(t/20 + 715.17 * i), 
                cy/3 + 7 * msin(t/20 - 915.31 * i), 
    ...
    hsl.x += mod2( (sw1 + field/3 + gala.x/5) * 0.8 + sin(t/20)/5, 1 ) ;
    hsl.y = max(hsl.y * 0.5, 0.2);
    //hsl.z -= hsl.x += swirl(cx, cy, 0.2, 1);
    ...
    {
        g = t/3.3 + i/23;
        t2 = 2*mod2(g, 1) - pow(mod2(g, 1), 2);
        
        gNoise += 
            max(0, min(1, 
                star(
                    cx - cos(floor(g) + i * ang) * 35*t2, 
                    cy + sin(floor(g) + i * ang) * 35*t2, 
                    0.4, 
                    8*t2*t2
                    )
                    *
                    max(0, (1 - pow(2*t2 - 1, 6)))
                ))
            ;
    }

    hsl.x = mod2(hsl.x + gNoise/2, 1);
